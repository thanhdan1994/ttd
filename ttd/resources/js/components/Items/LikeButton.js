...
function LikeButton({ login, showModalLogin, liked, like, unliked, unlike, id, handleLikeUnlike }) {
    async function handleLikeProduct(event) {
        ...
        const response = await PostService.doLikeProduct(id);
    }
    if (!login) {
        return (
            <button className="btn-like btn-outline-primary" onClick={showModalLogin}>{like} <i className="far fa-thumbs-up"></i>
            </button>
        )
    } else {
        return (
